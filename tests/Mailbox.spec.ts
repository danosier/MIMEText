...
import { Mailbox, MailboxAddrObject } from '../src/Mailbox'

const input1 = 'test@mail.com'
const input2 = `Test Lorem Ipsum <test@mail.com>`
const input3: MailboxAddrObject = {addr: 'test@mail.com', name: 'Test Lorem Ipsum', type: 'From'}

describe('Mailbox test suite', () => {
    ...
    test('gets no domain', () => {
        expect(new Mailbox('test').getAddrDomain()).toBe('')
    })
    ...
    // test('parse', () => {
    //     const fn = () => {
    //         new Mailbox({addr: 'fdlknfdkljdsnf'})
    //     }
    //
    //     expect(fn).toThrow()
    // })
